<?php
			...
			function getReport($analytics, $firstDay, $today, $VIEW_ID) {
			  ...
			  //$request->setDimensions($hostname);	
...
?>
        ...
        $sub1 = array_slice($netPull, 100, 200, true);
        ...
        foreach($sub1 as $key => $value) {
                ...
                //$getHostname = $result['reports'][0]['data']['rows'][0]['dimensions'][0];
				echo '<tr class="site-row">';
				echo '<td class="cell">' . $key . '</td>';
				echo '<td colspan="2" class="cell result sessions">' . intval($sessionsThisMonth) . '</td>';
				echo '<td class="cell result organic">' . $organicSearches . '</td>';
				echo '</tr>';
...
echo '<p style="font-size: 12px; color: #fff;"> | Analytics #3 gathered by: '.round(microtime(true) - $start, 4).' seconds</p>';
